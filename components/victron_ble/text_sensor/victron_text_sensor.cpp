...
void VictronTextSensor::publish_state_(VE_REG_DEVICE_STATE val) {
  switch (val) {
    ...
    case VE_REG_DEVICE_STATE::NOT_AVAILABLE:
      ...
      break;
    default:
      ESP_LOGW(TAG, "[%s] Unknown device state (%u).", this->parent_->address_str().c_str(), (uint8_t) val);
...
void VictronTextSensor::publish_state_(VE_REG_CHR_ERROR_CODE val) {
  switch (val) {
    ...
    case VE_REG_CHR_ERROR_CODE::NOT_AVAILABLE:
      ...
      break;
    default:
      ESP_LOGW(TAG, "[%s] Unknown device error (%u).", this->parent_->address_str().c_str(), (uint8_t) val);
      this->publish_state(to_string((uint8_t) val));
      break;
  }
}
